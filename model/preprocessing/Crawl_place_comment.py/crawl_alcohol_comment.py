...
def main():
    ...
    df = pd.read_csv('sampled_alcohol.csv',
        encoding="utf-8-sig",
        sep="|",
    )
    subcategory = df["분류"]
    ...
    for i in range(place_len):
        ...
        # 누적
        if not os.path.exists("alcohol_reviews.csv"):
            data.to_csv("alcohol_reviews.csv", index=False, sep="|", mode="w")
        else:
            data.to_csv(
                "alcohol_reviews.csv", index=False, sep="|", mode="a", header=False
            )
        ...
        if not os.path.exists("alcohol_review_num.csv"):
            review_num_data.to_csv("alcohol_review_num.csv", index=False, sep="|", mode="w")
        else:
            review_num_data.to_csv("alcohol_review_num.csv", index=False, sep="|", mode="a", header=False)
