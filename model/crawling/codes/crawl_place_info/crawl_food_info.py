...
## '음식점'으로 검색시 카페와 중복, 카카오맵에선 카테고리별 분류결과를 검색필터에서 제공하지 않으므로 따로 추출필요

gu_list = ["서귀포시", "제주시"]
food_category = ['한식','양식','중식','일식','뷔페','퓨전요리','아시아음식']  

#간식 = 제과/베이커리, 도넛,토스트, 초콜릿 등
#제외한 카테고리 = 치킨, 간식, 분식, 패스트푸드, 도시락, 샤브샤브, 구내식당, 야식, 기사식당, 푸드코트, 철판요리, 샐러드, 패밀리레스토랑
...
for index, gu_name in enumerate(gu_list):
    ...
    for food in food_category:
        ...
        #맥 : user-agent=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/92.0.4515.131 Safari/537.36 
        #윈도우 : user-agent=Mozilla/5.0 (Macintosh; Intel Mac OS X 10_12_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/61.0.3163.100 Safari/537.36
        chromedriver_path = "chromedriver"
        driver = webdriver.Chrome(
            os.path.join(os.getcwd(), chromedriver_path), options=options)  # chromedriver 열기
        driver.get("https://map.kakao.com/")  # 주소 가져오기
        search_area = driver.find_element_by_xpath(
            '//*[@id="search.keyword.query"]'
        )  # 검색 창
        search_area.send_keys(gu_name + ' ' + food)  # 검색어 입력
        driver.find_element_by_xpath('//*[@id="search.keyword.submit"]').send_keys(
            Keys.ENTER
        )  # Enter로 검색
        driver.implicitly_wait(10)  # 기다려 주자
